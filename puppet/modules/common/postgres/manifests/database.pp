...
define postgres::database($ensure, $owner = false, unicode = false ) {
    $ownerstring = $owner ? {
        false   => '',
        default => "-O $owner"
    }
    $unicodestring = $unicode? {
        false   => '',
        default => "--template template0 --encoding $unicode"
    }

    case $ensure {
        present: {
            exec { "Create $name postgres db":
                command => "/usr/bin/createdb $ownerstring $name $unicodestring",
                user    => 'postgres',
